...
df.isna().mean().sort_values(ascending=False)[df.isna().mean().sort_values(ascending=False) > 0]


#@ 4 
# Which columns contain more than 40% NA values?
# (compute a NumPy array of the column names, sorted alphabetically)
np.array(df.columns[(df.isna().mean() > .40) == True].sort_values())
...
# (compute a Series)
df['contbr_employer'].value_counts().sort_values(ascending=False)[:15]
...
# (compute a DataFrame)
def getMostValue(columnsName):
    return pd.Series(data = columnsName.value_counts().keys()[:15])

df[['contbr_employer', 'contbr_occupation', 'contbr_city']].apply(getMostValue, axis = 0)
...
np.array(df.contbr_occupation[df.contbr_occupation.str.contains('SELF', na = False)])
...
# (write a pd.DataFrame.dropna() statement)
df.dropna(subset=['election_tp'], how='all', inplace=True)
...
# (compute a number)
(df['contbr_zip'].str.isnumeric()[df['contbr_zip'].str.isnumeric() == False]).shape[0]


#@ 16
# What fraction of contb_receipt_amt values are less than 0?
# (compute a number between 0 and 1)
(df['contb_receipt_amt'].values < 0).mean()
